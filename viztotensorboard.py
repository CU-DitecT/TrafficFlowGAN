...
parser.add_argument('--experiment_dir', default='experiments/ngsim_learning_z_MO',
                    help="Directory containing 'test' ")
parser.add_argument('--result_dir', default='experiments/ngsim_learning_z_MO/test_result/ngsim_learning_z_MO/',
                    help="folder to viz")
...
writer = SummaryWriter(os.path.join(args.experiment_dir, "summary"))
...
def plot_pred_data(y_pred_variable, Exact_variable, variable, viz_dir, X_star, x, t, X, T, idx):
    ...
    ax.plot(X_variable_train[:, 1:2], X_variable_train[:, 0:1], 'kx',
            label='Data (%d points)' % (variable_train.shape[0]), markersize=1, clip_on=False)
    ...
    writer.add_figure(save_at+ '_data', fig)
    writer.close()
    ...
    # fig.tight_layout()
    writer.add_figure(save_at + '_prediction', fig)
    writer.close()
    ...
    writer.add_figure(save_at + '_uncertainty ', fig)
    writer.close()
...
def main(experiment_dir, result_dir, mode="debug", sudoku=True, interval="prediction",
    ...
    X_star = np.hstack(
        (X.flatten()[:, None], T.flatten()[:, None]))  # hstack is column wise stack, 241*960 (after flatten) by 2
