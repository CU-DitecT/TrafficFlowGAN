...
class GaussianLWR(torch.nn.Module):
    def __init__(self, meta_params_value, meta_params_trainable, lower_bounds, upper_bounds, hypers,
        ...
        for k, v in meta_params_value.items():
            if meta_params_trainable[k] == "True":
                ...
                self.torch_meta_params[k] = torch.nn.Parameter(torch.tensor(v, dtype=torch.float32),
                                                               requires_grad=False,
                                                               )
    ...
    def get_residuals(self, model, x_unlabel):
        ...
        x = torch.tensor(x_unlabel[:, 0:1], requires_grad=True).float().to(model.device).repeat(self.hypers["n_repeat"],
                                                                                                1)
        t = torch.tensor(x_unlabel[:, 1:2], requires_grad=True).float().to(model.device).repeat(self.hypers["n_repeat"],
                                                                                                1)
        ...
        r_mean, drho_dx, drho_dt, eq_2, r = self.caculate_residual_V2(rho, u, x, t, torch_params["umax"],
                                                                      torch_params["rhomax"], torch_params["tau"],
                                                                      model)
...
class GaussianARZ(torch.nn.Module):
    ...
    def caculate_residual(self, rho, u, x, t, Umax, RHOmax, Tau, model):
        ...
        drho_dt = torch.autograd.grad(rho, t, torch.ones([t.shape[0], 1], device=self.device).to(model.device),
                                      retain_graph=creat_graph, create_graph=creat_graph)[0]
        # print(f"one autograd time: {time.time() - start_time:.5f}")
        ...
        # added by ZM 0327. mask for U_eq - u
        mask = ((U_eq-u) > torch.tensor(-0.1)) * (U_eq < torch.tensor(0.1))
        f_u[~mask] = (U_eq - u)[~mask]
    ...
    def get_residuals(self, model, x_unlabel):
        ...
        f_rho_mean, f_u_mean, drho_dt = self.caculate_residual(rho, u, x, t, torch_params["umax"],
                                                               torch_params["rhomax"], torch_params["tau"], model)
...
class GaussianLWR_soft(GaussianLWR):
    def __init__(self, meta_params_value, meta_params_trainable, lower_bounds, upper_bounds, hypers,
                 train=False,
                 device=None):
        super(GaussianLWR_soft, self).__init__(meta_params_value, meta_params_trainable, lower_bounds, upper_bounds,
                                               hypers,
                                               train=train,
                                               device=device)
    ...
    def get_residuals(self, model, x_unlabel):
        ...
        r_mean, drho_dx, drho_dt, r1, r2 = self.caculate_residual(rho, u, x, t, torch_params["umax"],
                                                                  torch_params["rhomax"], torch_params["tau"], model)
